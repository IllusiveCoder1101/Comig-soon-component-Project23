...
function App() {
  ...
  return (
    <div className='box'>
      ...
      <div className="text">
      <h1 className='head'>We are launching <em>soon!</em></h1>
      <p className='about'>Subscribe and get notified</p>
      <form onSubmit={handleSubmit} className="form">
        <div className={(error)?"input error-input":"input"}>
        <input type="text" name="email" value={tmp} onChange={(e)=>setTmp(e.target.value)} className={(!error)?"email":"error"} placeholder={(!error)?'Email Address':"example@gmail.com"} />
        </div>
        {(error )?<p className='error-message'>Please Provide a valid Email Address.</p>:<p className='success'>{(active)?"Successful Submission":""}</p>}
        <button type='Submit' onClick={()=>setActive(true)} className="btn">Notify Me</button>
      </form>
      
      <img className="img" src={require("./assets/illustration-dashboard.png")} alt="" />
      <div className="icons">
